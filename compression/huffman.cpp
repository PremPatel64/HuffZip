...
// ----------------- JSON printing utilities for visualization -----------------
static string escape_symbol(uint8_t sym) {
    if (sym == '\n') return "\\n";
    if (sym == '\r') return "\\r";
    if (sym == '\t') return "\\t";
    if (sym == '\\') return "\\\\";
    if (sym == '\"') return "\\\"";
    if (sym >= 32 && sym <= 126) return string(1, (char)sym);
    // hex display for non-printables
    stringstream ss;
    ss << "0x" << setfill('0') << setw(2) << uppercase << hex << (int)sym;
    return ss.str();
}

static void print_node_json_recursive(ostream& out, const shared_ptr<HuffmanNode>& node, const unordered_map<uint8_t, string>& codes) {
    if (!node) {
        out << "null";
        return;
    }
    out << "{";
    out << "\"freq\":" << node->freq << ",";
    out << "\"isLeaf\":" << (node->is_leaf() ? "true" : "false");
    if (node->is_leaf()) {
        out << ",\"symbol\":" << static_cast<int>(node->symbol);
        out << ",\"symbolDisp\":\"" << escape_symbol(node->symbol) << "\"";
        auto it = codes.find(node->symbol);
        string code = (it != codes.end()) ? it->second : "";
        out << ",\"code\":\"" << code << "\"";
    } else {
        out << ",\"left\":";
        print_node_json_recursive(out, node->left, codes);
        out << ",\"right\":";
        print_node_json_recursive(out, node->right, codes);
    }
    out << "}";
}
...
public:
    ...
    // Visualize file structure as JSON output
    static void generate_visualization_json(const string& in_path) {
        vector<uint8_t> input_bytes = read_all_bytes(in_path);
        ...
        for (size_t i = 0; i < 256; ++i) {
            if (freq[i] > 0) {
                ...
                first = false;
                cout << "    {\"byte\":" << i 
                     << ",\"char\":\"" << escape_symbol(static_cast<uint8_t>(i)) << "\""
                     << ",\"count\":" << freq[i] << "}";
            }
        }
        cout << "\n  ],\n";
        ...
        for (size_t i = 0; i < 256; ++i) {
            if (freq[i] > 0) {
                ...
                if (!first) cout << ",\n";
                first = false;
                cout << "    \"" << static_cast<int>(sym) << "\":\"" << codes[sym] << "\"";
            }
        }
        cout << "\n  },\n";

        cout << "  \"tree\": ";
        print_node_json_recursive(cout, tree.root, codes);
        cout << "\n}\n";
    }
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        if (mode == "-v") {
            if (argc != 3) {
                cerr << "ERROR: Visualize mode requires exactly one input file.\n";
                print_usage(argv[0]);
                return 1;
            }
            string in_path = argv[2];
            HuffmanCodec::generate_visualization_json(in_path);
        } else {
            if (argc != 4) {
                print_usage(argv[0]);
                return 1;
